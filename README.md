...
If you desire to give a quick test of the program, you can run in your local machine `install-dev.ps1`; for linux/macOS developers, you can run `install-dev.sh`

```bash
# run this while being in the project's root.
bash script/install-dev.sh
```
...
- `list [sdkname] <flag>`: This will list every single SDK installed in your machine. You can specify an sdk name to check every version of that specific SDK. You can run `--available` flag, which allows see the available SDKs, and if you specify the SDK name, you can see the available versions of the SDK.
